...
#define lcl_vect_get_index(vect, idx, isize) (((char*)(vect))+((idx)*(isize)))
...
static lcl_err_t lcl_vect_expand( lcl_vect_t* vect, size_t min_size ) {
    ...
    lcl_vect_header_t* newh = realloc( header, new_cap * header->isize );
...
lcl_err_t __lcl_vect_inserts(lcl_vect_t *vect, size_t index, const void *data, size_t items)
{
    if (lcl_vectptr_valid(vect)) {
        ...
        memmove( lcl_vect_get_index( *vect, index + items, header->isize ), lcl_vect_get_index( *vect, index, header->isize ), (header->len-index) * header->isize );
        memcpy(  lcl_vect_get_index( *vect, index, header->isize ), data, items * header->isize );
        header->len += items;
    } else return LCL_BAD_ARGUMENT;
    return LCL_OK;
}
...
lcl_err_t __lcl_vect_splice(lcl_vect_t *vect, size_t index, void *data, size_t count)
{
    if (lcl_vectptr_valid(vect)) {
        ...
        if (data) {
            memcpy(  data, lcl_vect_get_index( *vect, index, header->isize ), count * header->isize );
        }
        
        memmove( lcl_vect_get_index( *vect, index, header->isize ), lcl_vect_get_index( *vect, index + count, header->isize ), (header->len-(index+count)) * header->isize );
        header->len -= count;
    } else return LCL_BAD_ARGUMENT;
    return LCL_OK;
}
...
lcl_err_t lcl_vect_swap(lcl_vect_t vect, size_t a, size_t b)
{
    if (vect) {
        ...
        void* aptr = lcl_vect_get_index( vect, a, header->isize );
        void* bptr = lcl_vect_get_index( vect, b, header->isize );
...
lcl_err_t __lcl_vect_truncate(lcl_vect_t *vect, void *dest, size_t count)
{
    if (lcl_vectptr_valid(vect)) {
        ...
        if (dest) {
            memcpy(
                dest, 
                lcl_vect_get_index( *vect, header->len-count, header->isize ) , 
                header->isize * count 
            );
        }
        header->len -= count;
